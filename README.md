...
目录页搜索框按名称即时筛选当前目录中的文件和文件夹，结果直接显示在目录列表或照片墙中，清空搜索框即可恢复显示。

路径栏搜索支持文件、文件夹名称或路径，点击文件夹结果可进入目录：

- 输入名称时，从根目录搜索。
- 输入绝对路径时，直接返回范围内的精确文件或文件夹；目标不存在但父目录存在时，仅在该父目录按名称片段搜索。路径超出范围或父目录不存在时显示无结果。
...
分享访问的搜索根目录为分享目录。路径搜索以性能优先，最多获取 50 个候选后排序，不保证覆盖所有更相似的文件或文件夹。
...
点击正文批注可展开对应讨论，评论中的“查看原文”可反向定位并保持高亮；同一处有多条讨论时，
可用上一条、下一条切换。桌面端评论面板自动随阅读位置显示相关讨论；移动端查看
原文后可点击“返回讨论”。协作者更新评论时，尚未提交的回复和消息修改会保留。

范围批注同时保存所在段落及其前后文。正文改写后，能确定所在段落的评论显示“原文已修改 · 已关联段落”，
并保留评论时的原文引用；不能可靠定位时显示“原文关联已失效”，隐藏“查看原文”入口。
已解决评论会折叠讨论内容，仍可查看处理回复、定位正文或重新打开。
...
目录页先显示页面框架和对应视图的加载占位，再通过 `?mode=directory-entries` 获取当前目录数据。
列表和 Gallery 保持连续滚动，只挂载视口及上下邻近区域的文件条目；排序、筛选、全选、批量操作和图片预览
使用完整目录数据。缩略图加载期间保留图片格子的尺寸，目录请求失败时可点击重新加载。
...
大目录性能基准会在临时目录生成 100 和 10,000 张图片，启动独立服务，输出首屏时间与 DOM 数量：

```bash
cargo build --release
node tests/browser/benchmark-directory.cjs
```
